...
minetest.register_on_dignode(function(pos, oldnode, digger)
	...
	if oldnode.name == "default:tree" then
		if not journal.playerdata_getKey(name,"journal_foundLog") then
			journal.add_entry("singleplayer","journal test","Today I found a log. It's nothing but a little peice of my ship I will have.")
...
minetest.register_on_craft(function(itemstack, player, old_craft_grid, craft_inv)
	...
	if itemstack:get_name() == "default:wood" then
		...
		if not journal.playerdata_getKey(name,"journal_craftedPlanks") then
			journal.add_entry("singleplayer","journal test","So I have some planks, but I will need a lot more to build my ship.")
